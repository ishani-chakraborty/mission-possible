...
class Histogram extends Component {
	constructor(props) {
		...
		this.data2 =
			this.passed_data.scenario_to_compare.map((x) => getMetric(x)) ?? [];
		...
		if (this.abs_max === Infinity || this.abs_max === -Infinity) {
			this.abs_max = 1;
		}
		...
		const options = (id) => {
			...
			return {
				...
				tooltip: {
					...
					x: {
						formatter: (x) => {
							let left = roundToDecimals(
								x - 0.5 * this.interval,
								2
							);
							let right = roundToDecimals(
								x + 0.5 * this.interval,
								2
							);
							return (
		...
		this.state = {
			...
			series_tuple: [
				{ name: " ", data: this.bins1 },
				{ name: " ", data: this.bins2 },
			],
	...
	static getStatistics(data) {
		...
		if (data.length !== 0) {
			...
			if (sorted.length % 2 === 1) {
				...
				//even length, avg the 2 midpoints
	...
	static dataToBins(data, numOfBuckets, min, max, interval) {
		...
		for (let i = 0; i < bins.length; i++) {
			final_bins.push({
				x: bins[i].label,
				y: data.length === 0 ? 0 : (100 * bins[i].count) / data.length,
			});
		}
